function CGame(oData) {
  ...
  this._init = function () {
    ...
    _oGoalKeeper = new CGoalKeeper(
      CANVAS_WIDTH_HALF - 100,
      CANVAS_HEIGHT_HALF - 225,
      _oContainerGame
    );
  ...
  this.onPressUp = function () {
    ...
    if (fForceLength > HIT_BALL_MIN_FORCE) {
      ...
      _vHitDir.set(
        -vHitDir2D.getX() * FORCE_MULTIPLIER_AXIS.x,
        fForceY,
        vHitDir2D.getY() * FORCE_MULTIPLIER_AXIS.z
      );
  ...
  this.addScore = function (iScore, iScoreNoMult) {
    _iScore += iScore;
    _oInterface.refreshTextScoreBoard(
      _iScore,
      _fMultiplier.toFixed(1),
      iScoreNoMult,
      true
    );
    console.log(iScore); //ovde hvatamo score
  };
  ...
  this.rejectBall = function () {
    ...
    switch (_iArea) {
      case 12:
        _oBall.getPhysics().velocity = _oBall
          .getPhysics()
          .velocity.vadd(
            new CANNON.Vec3(
              _oBall.getPhysics().velocity.x * 0.4,
              _oBall.getPhysics().velocity.y * 0.4,
              _oBall.getPhysics().velocity.z * 0.4
            )
          );
        break;
  ...
  this.goalProbability = function () {
    ...
    for (var i = 0; i < CALCULATE_PROBABILITY.length; i++) {
      if (
        _vHitDir.z < CALCULATE_PROBABILITY[i].zMax &&
        _vHitDir.z > CALCULATE_PROBABILITY[i].zMin
      ) {
        if (
          _vHitDir.x < CALCULATE_PROBABILITY[i].xMax &&
          _vHitDir.x > CALCULATE_PROBABILITY[i].xMin
        ) {
          _iArea = i;
        }
      }
  ...
  this.ballOut = function () {
    if (!_bBallOut && !_bGoal && !_bSaved) {
      ...
      if (
        oPos.y > BALL_OUT_Y ||
        oPos.x > BACK_WALL_GOAL_SIZE.width ||
        oPos.x < -BACK_WALL_GOAL_SIZE.width
      ) {
        _bBallOut = true;
  ...
  this.animGoalKeeper = function () {
    if (_bLaunched) {
      if (_bAnimGoalKeeper) {
        ...
        if (!_bAnimGoalKeeper) {
          _oGoalKeeper.viewFrame(
            _oGoalKeeper.getAnimArray(),
            _oGoalKeeper.getAnimArray().length - 1
          );
